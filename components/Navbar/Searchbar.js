        ...
        <View>
            ...
            {show && <View style={searchStyles.container}>
                <View style={searchStyles.subContainer}>
                    ...
                    {stores.map(store => {
                        return <TextLato key={Math.random()} style={{marginVertical: 3, fontSize: RFPercentage(1.5), color: gStyles.primary_medium}}>{store.title}</TextLato>
                    })}
                ...
                <View style={searchStyles.subContainer}>
                    ...
                    {products.map(product => {
                        return  <TouchableOpacity key={Math.random()} onPress={() => navigation.push('Product', {product})}>
                                    <TextLato style={{marginVertical: 3, fontSize: RFPercentage(1.5), color: gStyles.primary_medium}}>{product.title[language]}</TextLato>
                                </TouchableOpacity>
                    })}
                ...
                <View style={searchStyles.subContainer}>
                    ...
                    {categories.map(category => {
                        return <TouchableOpacity key={Math.random()} onPress={() => navigation.push('Category', category)}>
                                    <TextLato style={{marginVertical: 3, fontSize: RFPercentage(1.5), color: gStyles.primary_medium}}>{category.name[language]}</TextLato>
                                </TouchableOpacity>
                    })}
                ...
                <View style={searchStyles.subContainer} onPress={() => navigation.push('Subcategory', subcategory)}>
                    ...
                    {subcategories.map(subcategory => {
                        return <TextLato key={Math.random()} style={{marginVertical: 3, fontSize: RFPercentage(1.5), color: gStyles.primary_medium}}>{subcategory.name[language]}</TextLato>
                    })}
